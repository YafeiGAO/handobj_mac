...
parser.add_argument('-c', action="store_true", dest="convolutional", help='Whether use convolution or not')
parser.add_argument('-f', action="store_true", dest="fire", help='Whether use fire module or not')
parser.add_argument('-v', action="store_true", dest="variational", help='Wether use latent variance or not')
parser.add_argument('-m', action="store_true", dest="metric", help='Whether use metric loss or not')
parser.add_argument('-r', action="store", type=int, dest="rank", help='Rank of metric learning')
parser.add_argument('-o', action="store", dest="output_path", default="result_vae", help='Destination for storing results')
parser.print_help()
...
train_para = {'lr': [1e-5, 1e-6, 1e-7],
        ...
        'show_loss_freq': 1000,
        'snapshot_freq': 5000,
        'snapshot_dir': 'snapshots_handsegnet_vae_0625'}
dataset = BinaryDbReader(mode='training', batch_size=batch_size, shuffle=True, hue_aug=True, random_crop_to_size=True)
# build network graph
data = dataset.get()
print(data)
#import ipdb
...
for i in range(train_para['max_iter']):
    ...
    if (i % train_para['show_loss_freq']) == 0:
        print('Iteration %d\t Loss %.1e' % (i, train_cost))
