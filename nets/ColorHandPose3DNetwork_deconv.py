...
class ColorHandPose3DNetwork(object):
    ...
    def inference(self, image, hand_side, evaluation):
        ...
        print('result fr detection:')
        print(hand_scoremap)

        hand_scoremap = hand_scoremap['score_mask'][-1]
        print('[-1]')
        print(hand_scoremap)
    ...
    def inference2d(self, image, mask):
        ...
        # use network for hand segmentation for detection
        im = image
        hand_score = self.inference_detection(image)
        #hand_score = hand_score['score_mask']
        hand_score_pred  = hand_score['score_mask']
        print(hand_score)
        #hand_scoremap =  hand_score[-1]
        hand_scoremap = mask
        #print(hand_scoremap)
        # Intermediate data processing
        hand_mask = single_obj_scoremap(hand_scoremap)
        ...
        s = image_crop.get_shape().as_list()
        keypoints_scoremap = self.inference_pose2d(image_crop)
        #keypoints_scoremap = keypoints_scoremap[-1] #@@yafei
        #keypoints_scoremap = tf.image.resize_images(keypoints_scoremap, (s[1], s[2])) #@@yafei
        return {'image': im,
                'keypoints_scoremap': keypoints_scoremap, 
                'hand_score': hand_score_pred, 
    ...
    def inference_detection(image, train=False):
        ...
        return {'score_mask':scoremap_list_large, 'image':image}
