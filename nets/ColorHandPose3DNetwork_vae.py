...
class ColorHandPose3DNetwork(object):
    ...
    def inference(self, image, target, hand_side, evaluation):
        ...
        print('detection:')
        #hand_scoremap, lossz,loss_0d, loss_1d = self.inference_detection(image,label)
        crop_shape = [256,256]
        input_shape = [320,320,3]
        output_shape = [320,320,1]
        s = image.get_shape().as_list()
        print(s)
        s_t = target.get_shape().as_list()
        print('s_t')
        print(s_t)
        #ae['t'] = target
        #ae['x'] = image
        in_shape=[s[0]] + crop_shape + [input_shape[-1]]
        out_shape=[s[0]] + crop_shape + [output_shape[-1]]
        #x = tf.placeholder(tf.float32, in_shape, 'x')
        #t = tf.placeholder(tf.float32, out_shape, 't')
        ae = VAE(input_shape=in_shape,
        ...
        #target  = tf.placeholder(tf.float32, shape=[s[0]] + crop_shape + [output_shape[-1]])
        ae['x'] = image
        print('image.shape:')
        print(image)
        ae['t'] = target
        ...
        print(target)
        print(':detection')
        cost_handseg = ae['cost']
        hand  = ae['y']
        print(ae['t'])
        print(ae['y'])
        print(cost_handseg)
        hand = tf.cast(hand,dtype = bool)
        bg = tf.logical_not(hand)
        hand_scoremap = tf.cast(tf.concat([bg, hand], 3), tf.float32)
        print(hand_scoremap)
        ...
        return hand_scoremap, image_crop, scale_crop, center, keypoints_scoremap, keypoint_coord3d,cost_handseg
    ...
    def inference2d(self, image):
        ...
        """
        # use network for hand segmentation for detection
        #hand_scoremap = self.inference_detection(image,label)
        crop_shape = [256,256]                                                                  
        input_shape = [320,320,3]                                                           
        filter_sizes=[3, 3, 3, 3, 3, 3]
        output_shape = [320,320,1] 
        ...
        ae = VAE(input_shape=[None] + crop_shape + [input_shape[-1]],
                output_shape=1 + crop_shape + [output_shape[-1]],
                convolutional=True,
                fire=True,
                variational=True,
                metric=True,
        ...
        hand  = ae['y']
        bg = tf.logical_not(hand)
        hand_scoremap = tf.cast(tf.concat([bg, hand], 2), tf.int32)
        print(hand_scoremap)
        hand_scoremap = hand_scoremap[-1]
        ...
        keypoints_scoremap = keypoints_scoremap[-1]
        keypoints_scoremap = tf.image.resize_images(keypoints_scoremap, (s[1], s[2]))
        return keypoints_scoremap, image_crop, scale_crop, center, cost_handseg

    @staticmethod
    def inference_detection(image, label= [0], train=False):
        ...
        with tf.variable_scope('HandSegNet'):
            ...
            '''yafei'''
            scorevec = tf.reshape(scoremap,[scoremap.shape[0],scoremap.shape[1]*scoremap.shape[2]*scoremap.shape[3]]) #batch_size*2048
            score_full = tf.contrib.layers.fully_connected(scorevec, num_outputs)
            #print(label.shape)
            label = tf.squeeze(tf.cast(label,tf.int32),[1])
            nebula3d = tf.Variable(tf.truncated_normal([n_clusters, num_outputs]), 'nebula3d')

            z, z_mu, z_log_sigma, loss_z = variational_bayes(scorevec, n_code=num_outputs)
            '''
            print('z_mu')
            print(z_mu)
            print('label')
        ...
        return scoremap_list_large,loss_z, loss_0d, loss_1d
