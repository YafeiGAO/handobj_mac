...
class ColorHandPose3DNetwork(object):
    ...
    def inference_detection(image, train=False):
        ...
        with tf.variable_scope('HandSegNet'):
            ...
            nebula3d = tf.Variable(tf.truncated_normal([n_clusters, num_output]), 'nebula3d')
            z, z_mu, z_log_sigma, loss_z = variational_bayes(scorevec, n_code=num_output)
            loss_0d, loss_1d, loss_2d, loss_3d, _, _, nebula3d = sparse_ml(n_clusters, num_output, nebula3d, z, label, info_type='scalar')
            x = tf.reshape(z,[s3[0], s3[1],s3[2], s3[3]])
