...
class BinaryDbReader(object):
    ...
    def get(self):
        ...
        #label
        label_entries = 1
        record_bytes += encoding_bytes*label_entries
        #label
        ...
        # 7. label
        #label = tf.reshape(tf.slice(record_bytes_float32, [bytes_read//4], [label_entries]), [1])
        label = tf.slice(record_bytes_float32, [bytes_read//4], [label_entries])
        #print(bytes_read)


        bytes_read += encoding_bytes*label_entries
        ...
        hand_mask = tf.greater(hand_parts_mask, 0)#@@yafei: 1->0
        ...
        cond_l = tf.logical_and(tf.greater(hand_parts_mask, one_map), tf.less(hand_parts_mask, one_map))#@@yafei_0607:onemap*18 -> onemap
        cond_r = tf.greater(hand_parts_mask, zero_map)#@@yafei_0607:onemap*17 -> zeromap
        ...
        if self.scale_to_size:
            ...
            data_dict['image'], data_dict['hand_parts'], data_dict['hand_mask'],data_dict['label'] = tensor_stack_cropped[:, :, :3],\
